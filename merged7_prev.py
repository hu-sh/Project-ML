...
def main():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X), start=1):
            ...
            for lam in LAMBDA_SWEEP:
                z_ref = np.array(
                    [
                        ...
                        for z_i, pc2_i in zip(z_random_forest_train, pc2_train)
                    ]
                )
                y_pred = np.array([_reconstruct_y_from_z(z_i) for z_i in z_ref])
        ...
        z_random_forest_val_refined = _refine_with_lambda(
            z_random_forest_val, pc2_val, T, const, terms, z_min_ref, z_max_ref, best_lam, best_loss
        )
        ...
        for loss_mode in LOSS_MODES:
            ...
            z_refined = _refine_with_lambda(
                z_random_forest_val, pc2_val, T, const, terms, z_min_ref, z_max_ref, lam, loss_mode
            )
    ...
    if errors_by_loss:
        for loss_mode in LOSS_MODES:
            if errors_by_loss[loss_mode]:
                print(
                    f"MEE Totale ({loss_mode}, refined): {np.mean(errors_by_loss[loss_mode]):.5f}"
                )
