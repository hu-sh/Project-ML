...
\begin{document}
	...
	\begin{frame}{3.1 MONK Results: Summary Table}
		\begin{table}[]
			...
			\resizebox{\textwidth}{!}{
				\begin{tabular}{l|l|c|c}
					...
					\midrule
					\textbf{MONK 1} & $[10], 0.2, 0.9, 8\text{e-}5, \text{tanh}, 300$ & $2.0\text{e-}3 / 4.2\text{e-}3$ & $\mathbf{100\%} / \mathbf{100\%}$ \\
					\midrule
					\textbf{MONK 2} & $[10], 0.05, 0.9, 8\text{e-}5, \text{tanh}, 300$ & $2.5\text{e-}3 / 4.7\text{e-}3$ & $\mathbf{100\%} / \mathbf{100\%}$ \\
					\midrule
					\textbf{MONK 3 (Reg)} & $[10], 0.05, 0.9, 1\text{e-}4, ReLU, 600$ & $ 0.0592 / 0.04945$ & $93.4\% / \mathbf{97.2\%}$ \\
					\midrule
		...
		\vspace{0.2cm}
		\footnotesize
		\textbf{Key Observation (Monk 3):} The regularized model (ReLU, $\lambda=10^{-4}$) generalizes better on the clean Test Set ($97.2\%$) despite lower Training Accuracy ($93.4\%$), effectively filtering out the noise in the training data.
	\end{frame}
	...
	\begin{frame}{3.4 MONK 3 Results: Regularization Effect}
		\begin{columns}
			\column{0.5\textwidth}
			\centering
			\begin{figure} 
...
\begin{figure} 
	...
	\begin{frame}{4.4 CUP: Final Model (LBE Inverse Solver)}
	...
	\begin{frame}{4.7 CUP: Ensemble $z_{ens}$}
		...
		\begin{itemize}
			\item if $|z_{refined} - z_{forest\_refined}| < \alpha$, use their average
		...
		\begin{columns}
			...
			\begin{figure}
				\centering
				\includegraphics[width=\textwidth, height=4cm, keepaspectratio]{alpha_vs_mee_gamma_0.1.png}
