...
def compute_fold_outputs(X_train, X_val, Y_train, Y_val, z_train, z_val, T, const, terms):
...
def main():
    ...
    train_idx, test_idx = train_test_split(
        all_idx, test_size=0.1, random_state=random_state, shuffle=True
    )
    ...
    diffs = np.abs(z_recovered_refined - z_forest_refined)
    use_weighted = diffs < best_alpha
    z_ens = np.where(
        use_weighted,
        (1.0 - best_gamma) * z_recovered_refined + best_gamma * z_forest_refined,
        z_recovered_refined,
    )
    z_ens_refined = np.array(
        [
    ...
    mee_recovered = calculate_mee(fold["Y_val"], reconstruct_targets(fold["z_recovered"]))
    mee_refined = calculate_mee(fold["Y_val"], reconstruct_targets(z_recovered_refined))
    mee_forest = calculate_mee(fold["Y_val"], reconstruct_targets(fold["z_forest"]))
    mee_forest_refined = calculate_mee(fold["Y_val"], reconstruct_targets(z_forest_refined))
    mee_ens = calculate_mee(fold["Y_val"], reconstruct_targets(z_ens))
    mee_ens_refined = calculate_mee(fold["Y_val"], reconstruct_targets(z_ens_refined))
